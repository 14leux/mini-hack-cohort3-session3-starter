...
const SYSTEM_PROMPT =
  "You are Mini Hack Assistant. Use tools when they genuinely help; otherwise answer directly.";

async function connectChainkitMcp() {
  const url = process.env.CHAINKIT_MCP_URL;
  if (!url)
    throw new Error(
      "Set CHAINKIT_MCP_URL in your .env first, from the running mcp server output (e.g. http://localhost:2718/sse).",
    );
